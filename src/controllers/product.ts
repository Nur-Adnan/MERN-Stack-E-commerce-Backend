...
export const getlatestProducts = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has("latest-product"))
    ...
    await myCache.set("latest-product", JSON.stringify(products));
  }
...
export const getSingleProduct = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(`product-${id}`))
    ...
    await myCache.set(`product-${id}`, JSON.stringify(product));
  }
...
export const updateProduct = TryCatch(async (req, res, next) => {
  ...
  if (photos && photos.length > 0) {
    ...
    product.photos = photosURL as unknown as Types.DocumentArray<{
      public_id: string;
      url: string;
    }>;
  }
...
export const getAllProducts = TryCatch(
  async (req: Request<{}, {}, {}, SearchRequestQuery>, res, next) => {
    ...
    if (cachedData) {
      ...
      await myCache.set(key, JSON.stringify({ products, totalPage }));
    }
...
export const allReviewsOfProduct = TryCatch(async (req, res, next) => {
  ...
  if (myCache.has(key)) reviews = JSON.parse(myCache.get(key) as string);
    ...
    await myCache.set(key, JSON.stringify(reviews));
  }
